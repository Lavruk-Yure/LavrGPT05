# test_backtester_live_graph_orders_pytest.py
"""
Pytest-тест для перевірки графічного монітора та бек-тестера.
"""
...
def test_backtester_live_graph_orders():
    """Перевірка узгодженої роботи стратегії, бек-тестера й монітора."""
    ...
    for i, candle in enumerate(data):
        ...
        for order in open_orders[:]:
            if i - order["open_index"] >= bars_to_close:
                ...
                # order["profit"] = order.get(
                #     "profit", order["balance_after"] - order["balance_before"]
                # )
                bt.execution.close_order(order["id"], exit_price=price)
                ...
                if "profit" in order:
                    ...
                    # приблизна оцінка прибутку по різниці цін
