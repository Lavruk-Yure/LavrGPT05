"""
test_backtester_live_graph.py

Тест бек-тестера з SMA стратегією та графічним монітором.
Перевіряє логіку торгів та оновлення графіка.
"""
...
def backtester_setup():
    """
    Ініціалізація RiskManager, SMA стратегії, Backtester і графічного монітора.
    """
    ...
    return bt, rm, strategy, graph


def test_live_graph_backtester(backtester_setup):
    """
    Імітує торгівлю з закриттям ордерів через 3 бари,
    оновлює графічний монітор і перевіряє основні властивості.
    """
    bt, rm, strategy, graph = backtester_setup
    data = bt.load_data()
    open_orders = []
    ...
    assert rm.balance > 0, "Баланс має бути більшим за 0"
