# test_backtester_live_compact.py
"""
test_backtester_live_compact.py

Тест бек-тестера з SMA-стратегією і компактним монітором у LavrGPT05.

Задача:
- Перевірити логіку відкриття та закриття ордерів по сигналах.
- Оновлення монітора кожного бару.
- Підсумковий друк статистики через логер.
"""
...
from monitoring.monitor_live_compact import LiveMonitorCompact
...
def backtester_setup():
    """
    Ініціалізація RiskManager, LoggerMonitor, SMAStrategy, LiveMonitorCompact та
    Backtester з тестовими даними.
    """
    # Правильний шлях до data.csv у корені проєкту
...
def test_live_backtester_compact(backtester_setup):
    ...
    assert rm.balance != 10_000, "Очікується зміна балансу після торгів"
