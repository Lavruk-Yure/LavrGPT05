# test_backtester_live_compact_realtime.py
"""
test_backtester_live_compact_realtime.py

Тест бек-тестера з SMA-стратегією та live компактним монітором у режимі реального часу.
Перевіряє логіку торгів, закриття ордерів та оновлення монітора.
"""
...
from monitoring.monitor_live_compact_realtime import LiveMonitorCompactRealtime
...
def backtester_setup():
    """
    Ініціалізація RiskManager, LoggerMonitor, SMA стратегії,
    LiveMonitorCompactRealtime та Backtester з тестовими даними.
    """
    ...
    monitor = LiveMonitorCompactRealtime(refresh_delay=0.3)
...
def test_live_compact_realtime_backtester(backtester_setup):
    """
    Імітує торгівлю з SMA стратегією,
    закриває ордери через 3 бари,
    оновлює live компактний монітор,
    перевіряє базові умови про фінальний баланс.
    """
    ...
    # Закриття всіх відкритих ордерів в кінці
    for order in open_orders:
