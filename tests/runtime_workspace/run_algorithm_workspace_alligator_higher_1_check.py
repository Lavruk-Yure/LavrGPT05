# -*- coding: utf-8 -*-
"""Перевірка Alligator HIGHER_1 у multi-timeframe deterministic Replay."""
...
from core.workspace_alligator import (  # noqa: E402
    ...
    ALLIGATOR_CONFIRMATION_HIGHER_1_WARMUP,
    ALLIGATOR_REASON_HIGHER_1_BUY_ALLOW,
    ALLIGATOR_REASON_HIGHER_1_BUY_REJECT,
    ALLIGATOR_REASON_HIGHER_1_NOT_READY,
    ALLIGATOR_REASON_HIGHER_1_SELL_ALLOW,
    ALLIGATOR_REASON_HIGHER_1_SELL_REJECT,
    ...
    def create_synthetic_session(
        ...
        *,
        ...
        return WorkspaceReplaySession(
            events=self.events,
            source_name="ALLIGATOR_HIGHER_1_TEST",
...
def _assert_disabled_filter_ignores_stale_mode(
    ...
    assert algorithm.signal_filter.timeframe == "D1"
...
def main() -> None:
    ...
    assert signal_filter.timeframe == "H1"
    assert signal_filter.required_bars == ALLIGATOR_REQUIRED_BARS
    assert aggregator.source_timeframe == "M15"
    assert aggregator.target_timeframe == "H1"
    assert len(signal_filter.observations) == aggregator.completed_bars
    assert aggregator.completed_bars == 42
    assert aggregator.dropped_incomplete_buckets == 0
    assert aggregator.active_source_bars == 2
    assert all(item.timeframe == "H1" for item in signal_filter.observations)
    assert all(
        item.available_at == item.timestamp + timedelta(hours=1)
    ...
    assert tuple(
        ...
        ("ALLIGATOR", "H1", ALLIGATOR_REQUIRED_BARS),
    )

    expected_reasons = {
        ALLIGATOR_REASON_HIGHER_1_NOT_READY: 2,
        ALLIGATOR_REASON_HIGHER_1_BUY_ALLOW: 1,
        ALLIGATOR_REASON_HIGHER_1_SELL_REJECT: 1,
        ALLIGATOR_REASON_HIGHER_1_BUY_REJECT: 1,
        ALLIGATOR_REASON_HIGHER_1_SELL_ALLOW: 1,
    }
    actual_reasons = {
    ...
    assert all("alligator_timeframe=H1" in record.reason for record in records)
    ...
    print("Algorithm Workspace Alligator HIGHER_1 result")
    print("  base_timeframe=M15")
    print("  macd_timeframe=M15")
    print("  alligator_timeframe=H1")
    print(f"  macd_warmup={MACD_REQUIRED_BARS}")
    print(f"  alligator_warmup={ALLIGATOR_REQUIRED_BARS}")
    print("  completed_higher_bars=42")
    print("  incomplete_final_higher_bar_not_emitted=True")
    print("  signals=6")
    print("  allowed=2")
    print("  rejected=4")
    print("  higher_warmup_rejected=2")
    ...
    print("ALGORITHM_WORKSPACE_ALLIGATOR_HIGHER_1_CHECK=OK")
