...
from core.common_dialogs import CommonInfoDialog
...
class SettingsPageLanguage(QWidget):
    ...
    def __init__(
        ...
        # ВАЖЛИВО: інакше старий combo може "намалюватись" ще раз до deleteLater()
        old.hide()
        old.setParent(None)  # prevent "ghost" widget before deleteLater()
        old.deleteLater()
        ...
        self.ui.comboLanguage.activated.connect(self._on_user_language_activated)
        self._last_warn_code: str | None = None
    ...
    def _warn_if_translator_off(self) -> None:
        conf = session_state.CURRENT_CONFIG
        if conf is None:
            return
        ...
        provider = translator.get("provider", "off")

        if provider != "off":
            return  # перекладач увімкнений — мовчимо

        CommonInfoDialog.show_dialog(
            parent=self,
            lang_mgr=self._lang_mgr,
            title=self._lang_mgr.resolve("LanguageChangeWarning.windowTitle"),
            header=self._lang_mgr.resolve("LanguageChangeWarning.header"),
            details=self._lang_mgr.resolve("LanguageChangeWarning.details"),
        )

    def _on_user_language_activated(self, _idx: int) -> None:
        code = self.current_code() or ""
        if not code:
            return

        # Попередження показуємо тільки 1 раз на кожну мову за сесію
        if code != self._last_warn_code:
            self._warn_if_translator_off()
            self._last_warn_code = code

        log_cp("combo.user_activated", code=code)
