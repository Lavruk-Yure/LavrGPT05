...
# -*- coding: utf-8 -*-
"""
LicenseManager — логіка ліцензій LGE (офлайн, без license.json).

Важливе:
- Джерело правди: LGE.conf -> data["license"].
- Варіант machine_id: MachineGuid + SystemUUID (Windows).
- Free: trial на TRIAL_DAYS днів, потім TRIAL_EXPIRED.
- Pro: безстрокова валідна ліцензія -> PRO_OK.
- Pro+: безстрокова валідна ліцензія -> PRO_OK.
...
from core.app_paths import PROGRAM_DATA_DIR
...
def load_public_key_b64_from_resources() -> str:
    f = QFile(":/keys/_public_ed25519.b64")

    if not f.open(QIODevice.OpenModeFlag.ReadOnly):
        raise RuntimeError("_public_ed25519.b64 not found in resources")

    ba = f.readAll()  # QByteArray
    data_b = ba.data()  # bytes
    text = data_b.decode("utf-8", errors="replace")

    return text.strip()


PUBLIC_KEY_B64 = load_public_key_b64_from_resources()
...
class LicenseManager:
    ...
    def activate_key(
        ...
        *,
        ...
        edition = cls._norm_edition(payload.get("edition"))
        ...
        # machine binding: license must belong to this machine
        payload_machine_id = payload.get("machine_id") or payload.get("fingerprint")
        if not isinstance(payload_machine_id, str) or not payload_machine_id:
        ...
        if payload_machine_id not in machine_id_candidates:
            log_lp(
                "activate.other_machine",
                payload_machine_id=payload_machine_id,
                current_machine_id=machine_id_preferred,
            )
    ...
    def enable_trial(
        ...
        *,
        ...
        Якщо trial_lock уже існує — повторний trial заборонено.
        """
        now = now or datetime.now(UTC)
        lic = cls._ensure_license_block(conf)

        if cls._trial_lock_exists(conf):
            raise RuntimeError(cls.ST_TRIAL_ALREADY_USED)

        trial_days = cls._policy_int(lic, "trial_days", TRIAL_DAYS)
        lic_backup = deepcopy(lic)
        ...
        try:
            ...
            lic["activated_at"] = now.isoformat()
            lic["expires_at"] = (now + timedelta(days=trial_days)).isoformat()

            cls._write_trial_lock(conf, lic, now)
    ...
    def compute_machine_id_candidates(cls) -> tuple[str, set[str]]:
        ...
        preferred = hashlib.sha256(preferred_raw.encode("utf-8")).hexdigest()

        candidates = {preferred}

        # legacy: без SystemUUID
        legacy_raw = f"LGE|v1|{mg}|NA|{cls._SALT}"
        legacy = hashlib.sha256(legacy_raw.encode("utf-8")).hexdigest()
        candidates.add(legacy)

        return preferred, candidates

    @classmethod
    def _build_trial_lock_machine_hash(cls, conf: Dict[str, Any]) -> str:
        """
        Будує короткий machine-hash для шляху trial lock.

        Основа:
        - conf["machine"]["username"]
        - conf["machine"]["platform"]
        - conf["machine"]["node"]
        - conf["machine"]["mac"]

        Якщо machine-блок порожній — fallback на compute_machine_id().
        """
        machine = conf.get("machine", {})
        if not isinstance(machine, dict):
            machine = {}

        username = str(machine.get("username") or "").strip()
        platform = str(machine.get("platform") or "").strip()
        node = str(machine.get("node") or "").strip()
        mac = str(machine.get("mac") or "").strip()
        ...
        if not raw:
            raw = cls.compute_machine_id()

        return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:32]

    @classmethod
    def _get_trial_lock_path(cls, conf: Dict[str, Any]) -> Path:
        """
        Повертає шлях trial lock:

        %ProgramData%\\Microsoft\\CLR\\{hash}\\t.dat
        """
        machine_hash = cls._build_trial_lock_machine_hash(conf)
        return PROGRAM_DATA_DIR / "Microsoft" / "CLR" / f"{{{machine_hash}}}" / "t.dat"

    @classmethod
    def _trial_lock_exists(cls, conf: Dict[str, Any]) -> bool:
        """
        Повертає True, якщо trial lock уже існує.

        Якщо файл існує, навіть пошкоджений/кривий — вважаємо, що trial уже був.
        Це безпечніше, ніж дозволити повторний trial.
        """
        try:
            return cls._get_trial_lock_path(conf).exists()
        except Exception as e:  # noqa
            log_lp("trial_lock.exists.fail", err=str(e))
            return False
    ...
    def _write_trial_lock(
        ...
        Якщо lock уже існує — повторний trial заборонено.
        Якщо lock не вдається записати — trial не стартує.
        """
        path = cls._get_trial_lock_path(conf)

        if path.exists():
            raise RuntimeError(cls.ST_TRIAL_ALREADY_USED)

        # Створюємо весь шлях, але не чіпаємо атрибути Microsoft / CLR
        path.parent.mkdir(parents=True, exist_ok=True)

        payload = {
            "trial_used": True,
            "machine_id": str(lic.get("machine_id") or cls.compute_machine_id()),
            "first_trial_started_at": now.isoformat(),
        }
        ...
        # Ховаємо тільки НАШУ папку {hash}
        cls._set_hidden_system_attributes(path.parent)

        # Ховаємо тільки НАШ файл t.dat
        cls._set_hidden_system_attributes(path)

    @staticmethod
    def _set_hidden_system_attributes(path: Path) -> None:
        """
        Встановлює Windows-атрибути Hidden + System для файла/папки.

        На не-Windows нічого не робить.
        """
        if os.name != "nt":
            return
        ...
        set_file_attributes_w = kernel32.SetFileAttributesW
        set_file_attributes_w.argtypes = [wintypes.LPCWSTR, wintypes.DWORD]
        set_file_attributes_w.restype = wintypes.BOOL
    ...
    def _compute_status(
        ...
        *,
        ...
        if edition == "free":
            ...
            days_used = cls._days_used(lic, now, set_if_missing=False)
            expires_at = cls._parse_dt(lic.get("expires_at"))
            trial_days = cls._policy_int(lic, "trial_days", TRIAL_DAYS)

            if expires_at is None:
                expires_at = activated + timedelta(days=trial_days)
    ...
    def _caps_for_status(cls, status: str, edition: str, days_used: int) -> LicenseCaps:
        ...
        elif status in (cls.ST_OTHER_MACHINE, cls.ST_TRIAL_ALREADY_USED):
            live_manual = False
            live_semi = False
            auto_demo = False
            auto_full = False
    ...
    def _norm_edition(value: Any) -> str:
        if isinstance(value, str):
            v = value.strip().lower().replace("+", "_plus")
    ...
    def _verify_signature(cls, payload_b64: str, signature_b64: str) -> bool:
        try:
            ...
            # LGEOffice зараз може зберігати повний signed message:
            # signature(64 bytes) + payload_bytes.
            # Для cryptography Ed25519 потрібні тільки перші 64 байти підпису.
            if len(sig_bytes) > 64:
    ...
    @classmethod
    def is_trial_already_used(cls, conf: Dict[str, Any]) -> bool:
        """Публічний helper для UI: чи був trial уже використаний."""
        return cls._trial_lock_exists(conf)
