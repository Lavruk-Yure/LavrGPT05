# settings_center.py
"""
SettingsCenter — центр налаштувань LGE.

Призначення:
- показує сторінки налаштувань у форматі ліве меню + stackPages;
- не містить торгової логіки;
- читає/зберігає тільки налаштування через відповідні сторінки;
- сторінки IB/cTrader окремо не показуються;
- broker/account/execution налаштування переходять у сторінку
  "Торговий режим".

Поточний порядок сторінок:
    0) Language
    1) Translator
    2) Trading Mode
    3) License
...
from core.settings_page_trading import SettingsPageTrading
...
class SettingsCenter(QDialog):
    ...
    def _init_pages(self) -> None:
        ...
        self._page_trading = SettingsPageTrading(
            self._lang_mgr,
            parent=stack,
        )
        stack.addWidget(self._page_trading)  # index = 2

        self._page_license = SettingsPageLicense(
            stack,
            self._lang_mgr,
    ...
    def _init_tree_items(self) -> None:
        ...
        entries = [
            ...
            NavEntry("[SettingsCenter.tree.trading]", page_index=2),
            NavEntry("[SettingsCenter.tree.license]", page_index=3),
    ...
    def _on_nav_changed(self, current: QTreeWidgetItem, _prev: QTreeWidgetItem) -> None:
        ...
        if 0 <= page_i < self.ui.stackPages.count():
            ...
            if page_i == 3 and hasattr(self, "_page_license"):
                try:
    ...
    def _apply_language(self, code: str) -> None:
        """
        Застосовує вибрану мову:
        - дозаповнює відсутні переклади для активної мови;
        - зберігає активну мову в strings.json;
        - зберігає мову в LGE.conf, якщо є активна сесія;
        - оновлює переклад головного вікна та SettingsCenter.
        """
        ...
        # 1) strings.json: lang_active
        self._lang_mgr.set_current_language(code)

        # 2) LGE.conf (якщо є сесія)
        try:
        ...
        if hasattr(self, "_page_trading"):
            self._page_trading.snapshot()
    ...
    def _on_license_restart_requested(self) -> None:
        restart = getattr(
            self._main_window,
