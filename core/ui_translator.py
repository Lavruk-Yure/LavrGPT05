# ui_translator.py
"""
UITranslator — перекладач UI-елементів LGE.
...
from PySide6.QtWidgets import QGroupBox, QTreeWidget, QTreeWidgetItem, QWidget
...
class UITranslator:
    ...
    def _apply_group_box_title(self, group_box: QGroupBox) -> None:
        """
        Перекладає title у QGroupBox.

        QGroupBox не має text(), тому стандартна обробка QWidget
        його заголовок не перекладає.
        """
        raw = group_box.title()
        if not raw:
            return
        ...
        if key:
            group_box.setProperty("lge_tr_group_title_key", key)
            log_cp(
                "group.title.key.detected",
                key=key,
                widget=group_box.objectName(),
            )

        if not key:
            saved = group_box.property("lge_tr_group_title_key")
            key = saved.strip() if isinstance(saved, str) and saved.strip() else None
        ...
        if value:
            ...
            log_cp("group.title.translated", key=key, text=value)
